...
async function resetDanmakuLikes() {
  try {
    // 连接数据库
    ...
    // 更新前5条弹幕的点赞数和清空likedBy数组
    const likesArray = [10, 20, 40, 80, 100];

    for (let i = 0; i < 5; i++) {
      danmakus[i].likes = likesArray[i];
      danmakus[i].likedBy = []; // 清空点赞用户列表
      await danmakus[i].save();
      console.log(`更新弹幕 ${i + 1}: "${danmakus[i].text}" - 点赞数: ${likesArray[i]}, likedBy已清空`);
    }

    console.log('\n更新完成！所有测试弹幕的likedBy数组已清空，可以正常测试点赞功能。');
    process.exit(0);
  } catch (error) {
